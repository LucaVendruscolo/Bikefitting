...
class VideoSyncConfig:
    ...
    video_fps: float
    total_frames: int
    initial_direction: str = "right"  # "left" or "right" - bike direction at video start
    ...
    @classmethod
    def from_dict(cls, d):
        # Handle old configs without initial_direction
        if 'initial_direction' not in d:
            d['initial_direction'] = "right"
        return cls(**d)
...
class DatasetBuilderApp:
    ...
    def setup_ui(self):
        ...
        # Bike direction at video start (for angle calibration)
        direction_frame = ttk.Frame(sync_frame)
        direction_frame.pack(fill="x", pady=5)
        
        ttk.Label(direction_frame, text="Bike direction at video start:").pack(side="left")
        self.direction_var = tk.StringVar(value="right")
        ttk.Radiobutton(direction_frame, text="← Facing Left", variable=self.direction_var, 
                        value="left").pack(side="left", padx=10)
        ttk.Radiobutton(direction_frame, text="Facing Right →", variable=self.direction_var, 
                        value="right").pack(side="left", padx=10)
        ttk.Label(direction_frame, text="(perpendicular to camera)", 
    ...
    def on_video_select(self, event):
        ...
        try:
            ...
            if video_name in self.sync_configs:
                ...
                self.direction_var.set(config.initial_direction)
                ...
                self.direction_var.set("right")  # Default direction
                self.update_trim_label()
    ...
    def save_current_video_config(self):
        ...
        config = VideoSyncConfig(
            ...
            initial_direction=self.direction_var.get()
        )
    ...
    def create_dataset(self):
        ...
        output_dir = filedialog.askdirectory(
            ...
            initialdir=str(getattr(self, "default_output_path", self.base_path))
        )
        ...
        try:
            ...
            for idx, (video_name, config) in enumerate(self.sync_configs.items()):
                ...
                # === ANGLE CALIBRATION ===
                # Get the initial heading at the start of the trimmed video
                # The bike is perpendicular to the camera at video start
                fps = config.video_fps
                start_frame_offset = (config.trim_start_frame - config.sync_frame) / fps
                start_frame_time = sync_time + timedelta(seconds=start_frame_offset)
                
                # Find the sensor reading closest to the start frame
                time_diffs = abs(csv_df['wall_time_iso'] - start_frame_time)
                start_idx = time_diffs.idxmin()
                initial_heading = csv_df.iloc[start_idx]['euler_heading_deg']
                
                # Calculate the offset to make angles camera-relative
                # At video start: bike is perpendicular (±90° from camera)
                # If facing right: initial_heading should map to +90°
                # If facing left: initial_heading should map to -90°
                if config.initial_direction == "right":
                    # Bike pointing right = +90° from camera perspective
                    angle_offset = initial_heading - 90.0
                else:
                    # Bike pointing left = -90° from camera perspective
                    angle_offset = initial_heading + 90.0
                ...
                for frame_num in range(config.trim_start_frame, config.trim_end_frame + 1):
                    ...
                    # Get sensor data
                    sensor_row = csv_df.iloc[closest_idx]
                    
                    # === TRANSFORM ANGLE TO CAMERA-RELATIVE ===
                    # Raw heading is in global coordinates (0-360)
                    raw_heading = sensor_row['euler_heading_deg']
                    
                    # Convert to camera-relative angle
                    # Add 180° so that 0° = facing camera (not facing away)
                    camera_angle = raw_heading - angle_offset + 180.0
                    ...
                    # === BUILD OUTPUT ROW (only useful columns for angle prediction) ===
                    output_row = {
                        # Frame identification
                        'frame_path': str(frame_path.relative_to(output_path)),
                        'source_video': video_name,
                        'frame_number': frame_num,
                        
                        # TARGET: Camera-relative bike angle (-180 to 180, 0 = facing camera)
                        'bike_angle_deg': round(camera_angle, 2),
                        
                        # Useful sensor features for ML
                        'euler_roll_deg': sensor_row['euler_roll_deg'],      # Side tilt
                        'euler_pitch_deg': sensor_row['euler_pitch_deg'],    # Forward/back tilt
                        
                        # Gyroscope (rotation rates) - useful for motion prediction
                        'gyro_x_rad_s': sensor_row['gyro_x_rad_s'],
                        'gyro_y_rad_s': sensor_row['gyro_y_rad_s'],
                        'gyro_z_rad_s': sensor_row['gyro_z_rad_s'],
                        
                        # Linear acceleration (motion without gravity)
                        'lin_accel_x_m_s2': sensor_row['lin_accel_x_m_s2'],
                        'lin_accel_y_m_s2': sensor_row['lin_accel_y_m_s2'],
                        'lin_accel_z_m_s2': sensor_row['lin_accel_z_m_s2'],
                        
                        # Sync quality (for debugging/filtering)
                        'sync_time_diff_ms': round(time_diff_ms, 2),
                    }
            ...
            if all_data:
                ...
                # Columns are already in the right order from output_row
